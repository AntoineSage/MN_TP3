...
void test_float() {
    ...
    mncblas_sgemv(MNCblasRowMajor, MNCblasNoTrans, 2, 2, 5, A, 
0, X, 1, 6, Y, 1);
...
void test_complexe_float() {
    ...
    mncblas_cgemv(MNCblasRowMajor, MNCblasNoTrans, 2, 2, &c1, A, 
0, X, 1, &c2, Y, 1);

	printf("\n{");
	for (int i = 0; i < 2; i++) {
		printf("Re %f ", Y[i].real);
		printf("Im %f ; ", Y[i].imaginary);
	}
	printf("}\n");
}
